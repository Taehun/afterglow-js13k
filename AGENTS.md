...
- **Roadroller는 결정적이다 (파라미터 캐시)**: -O1 랜덤 탐색은 실행마다 ±20B
  ...
  `optimize(0)` 압축한다.
- **캐시 갱신은 반드시 "롤-베스트"로**: -O2 탐색도 확률적이라 **1회 실행이
  기존보다 나쁜 파라미터로 캐시를 덮어쓸 수 있다** (2026-08-20 두 번 실제 발생,
  +20B급 퇴행). 절차: 현 params 백업 → `build:max`를 3~5회 반복하며 매회
  `dist/size.json`의 zip을 비교 → 최소였던 회차의 params만 남기고 나머지는
  버린다. 퇴행했으면 `git checkout -- tools/roadroller-params.json`으로 복원.
